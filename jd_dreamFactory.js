...
[task_local]
#京喜工厂
10 * * * * https://raw.githubusercontent.com/shuye73/MyActions/main/scripts/jd_dreamFactory.js, tag=京喜工厂, enabled=true

================Loon==============
[Script]
cron "10 * * * *" script-path=https://raw.githubusercontent.com/shuye73/MyActions/main/scripts/jd_dreamFactory.js,tag=京喜工厂

===============Surge=================
京喜工厂 = type=cron,cronexp="10 * * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/shuye73/MyActions/main/scripts/jd_dreamFactory.js

============小火箭=========
京喜工厂 = type=cron,script-path=https://raw.githubusercontent.com/shuye73/MyActions/main/scripts/jd_dreamFactory.js, cronexpr="10 * * * *", timeout=200, enable=true
...
function updateTuanIds(url = 'https://raw.githubusercontent.com/shuye73/updateTeam/master/jd_updateFactoryTuanId.json') {
  return new Promise(resolve => {
...
function updateTuanIdsCDN(url = 'https://raw.fastgit.org/shuye73/updateTeam/master/jd_updateFactoryTuanId.json') {
  return new Promise(async resolve => {
    $.get({url,
      headers:{
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/shuye73/updateFactoryTuanId/master/jd_updateFactoryTuanId.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
